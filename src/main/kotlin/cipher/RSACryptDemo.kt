...
fun main(args: Array<String>) {
   ...
   /* val generator = KeyPairGenerator.getInstance("RSA")//秘钥对生成器
    ...
    println("byte 数组长度=" + input.toByteArray().size)
    ...
    //私钥加密：加密不能超过117个字节
    val encrypt = RSACryptDemo.encryptByPrivateKey(input, privateKey)
    println("私钥加密=" + encrypt)
    val encryptByPublicKey = RSACryptDemo.encryptByPublicKey(input, publicKey)
    println("公钥加密=" + encryptByPublicKey)

    val decryptByPrivateKey = RSACryptDemo.decryptByPrivateKey(encryptByPublicKey, privateKey)
    println("私钥解密=" + decryptByPrivateKey)

    val decryptByPublicKey = RSACryptDemo.decryptByPublicKey(encrypt, publicKey)
